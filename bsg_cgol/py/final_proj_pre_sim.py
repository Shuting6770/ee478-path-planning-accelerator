...
def get_bmp_zero_idxs(img_name):
  ...
  img_path = osp.join(work_dir_path, ('bsg_cgol/photos/' + img_name))
...
def send_game_req(fout, board: np.ndarray, length, width, max_len, start_end_point):
    '''
    Given a board, a game length, and start/end point, write this to the trace file so the
    data is sent to the DUT.
    '''
    game_len_formatter = f'{{:0{ceil(log2(max_len + 1))}b}}'
    game_len_bin = game_len_formatter.format(length)
    
    # Ensure start_end_point is a 24-bit binary string
    if isinstance(start_end_point, str) and len(start_end_point) == 24:
        start_end_point_bin = start_end_point
    else:
        raise ValueError("start_end_point should be a 24-bit binary string")

    fout.write(f'### sending board config (game length {length}, start_end_point {start_end_point_bin}) ###\n')
    ...
    while wr_idx < width**2:
        if wr_idx == 0:
            data = np.flip(board_in[0:(trace_data_width - len(game_len_bin) - len(start_end_point_bin))])
            wr_idx += (trace_data_width - len(game_len_bin) - len(start_end_point_bin))
            fout.write(f'0001____0_00000000000_{"".join([str(x) for x in data])}_{start_end_point_bin}_{game_len_bin}\n')
        else:
...
def main():
  ...
  start_end_point = cfg["board"]["start_end_point"]
  cgol_iterations = 0
  cgol_elapsed_time = 0
  with open(args.out, 'w') as fout:
    ...
    for (idx, game) in enumerate(cfg['games']):
      ...
      # Generate the trace for this game: only check the last frame
        ...
        send_game_req(fout, board, game['length'], width, max_len, start_end_point) # Send initial data
